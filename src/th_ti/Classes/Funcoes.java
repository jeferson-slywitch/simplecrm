...
//        numero = CasaDecimal(String.valueOf(Valor));
    ...
    public String NomePessoa(int Codigo){
        ...
        SQL = "select nome from t_pessoa where codigo = " + Codigo + " and exclusao = 0 and bloqueio = 0 ";
        Conexao = new ConexaoPostgresql();
        Conexao.Conectar() ;
        Conexao.setSQL(SQL);
        ResultSet RS = Conexao.Consultar();
    ...
    public String PalavreChave(int Codigo) {
        
        String SQL, strPalavraChave=null;
        SQL = "select nome from t_chave where codigo = " + Codigo + " and exclusao = 0 ";
        Conexao = new ConexaoPostgresql();
        Conexao.Conectar();
        Conexao.setSQL(SQL);
        ResultSet RS = Conexao.Consultar();
        ...
        try{
            if (RS.next()){
                strPalavraChave = (RS.getString("nome"));
            }
        }catch (Exception Erro){
            JOptionPane.showMessageDialog(null, "Erro na validação da função palavra chave: " + Erro);
        }
        
        Conexao.Desconectar();

        return strPalavraChave;
